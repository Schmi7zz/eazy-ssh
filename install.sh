...
# ║                                                             ║
# ║  github.com/YOUR_USERNAME/eazy-ssh                          ║
# ║  t.me/SchmitzWS                                             ║
# ╚══════════════════════════════════════════════════════════════╝
set -e
...
# If running from cloned repo, copy files; otherwise clone
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -f "$SCRIPT_DIR/backend/main.go" ]]; then
    cp -r "$SCRIPT_DIR" "$INSTALL_DIR"
    success "Copied from local directory"
else
    git clone https://github.com/YOUR_USERNAME/eazy-ssh.git "$INSTALL_DIR" > /dev/null 2>&1 &
...
# EazySSH - Backend WebSocket Proxy
server {
    listen 80;
...
echo ""
echo -e "  ${D}┌─────────────────────────────────────────────┐${N}"
echo -e "  ${D}│${N}  ${C}Frontend${N}    https://${FRONTEND_DOMAIN}  ${D}│${N}"
echo -e "  ${D}│${N}  ${C}Backend${N}     https://${BACKEND_DOMAIN}   ${D}│${N}"
echo -e "  ${D}│${N}  ${C}Mini App${N}    t.me/${BOT_USERNAME}/${APP_SHORT}  ${D}│${N}"
echo -e "  ${D}└─────────────────────────────────────────────┘${N}"
echo ""
echo -e "  ${W}Next steps:${N}"
echo ""
echo -e "  ${ARROW} Go to ${W}@BotFather${N} in Telegram and run:"
echo ""
